...
### 6.4 LAN 分享/二维码 —— 已具备（Web 端既有能力）

**现状**：v2rayA 既有分享弹窗（`modalSharing.vue`）已含二维码生成（节点/订阅分享链接 + `sub://` base64 形式）+ 复制链接 + 扫码导入（`handleClickImportConfirm` 直接识别分享链接）。Web 端无需 LAN 互传（访问面板即同一实例）。
...
未排期项：per-app 代理（Linux 网关场景价值有限）、新手模式、SS2022 核心实现（约 2~3 人日）、4.1 二期/三期完整路由规则 UI。
