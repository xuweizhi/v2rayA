...
// SkipSessionIDEchoCheck disables the legacy session ID echo verification in
// the TLS 1.3 client. Used by protocols that repurpose the session ID (e.g.
// ShadowTLS v3) while the peer relays the handshake of a server that echoes
// the modified session ID.
var SkipSessionIDEchoCheck atomic.Bool

// ClientHelloMarshalHook, if set, is called with the marshaled ClientHello
// message (excluding the TLS record header) right before it is hashed into
// the handshake transcript and written to the wire. It allows protocols that
// repurpose the legacy session ID (e.g. ShadowTLS v3) to rewrite the session
// ID bytes consistently on both the wire and the transcript.
var ClientHelloMarshalHook func(marshaledHello []byte)

// SessionIDProvider, if set, supplies the 32-byte legacy session ID of the
// ClientHello instead of a random one. Used by protocols that repurpose the
// session ID (e.g. ShadowTLS v3).
var SessionIDProvider func() [32]byte
